...
class Expediente {
    ...
    function consulta_exp($radicado) {
        ...
        $query = "select e.SGD_EXP_NUMERO,e.SGD_EXP_ESTADO,$radi_nume_radi AS RADI_NUME_RADI from SGD_EXP_EXPEDIENTE e where e.RADI_NUME_RADI = '$radicado'
    ...
    function insertar_expediente($num_expediente, $radicado, $depe_codi, $usua_codi, $usua_doc) {
        ...
        $estado_expediente = 0;
        $creadoOld = 0;
        $query = "select * from SGD_SEXP_SECEXPEDIENTES e WHERE e.SGD_EXP_NUMERO='$num_expediente'";
        $rs = $this->db->conn->query($query);
        if (!$rs->fields["sgd_exp_numero"]) {
            $this->db->conn->query($query);
            $query = "select e.SGD_EXP_NUMERO as SGD_EXP_NUMERO, e.SGD_EXP_FECH as SGD_EXP_FECH  from SGD_EXP_EXPEDIENTE e WHERE e.SGD_EXP_NUMERO='$num_expediente' order by e.SGD_EXP_FECH DESC";
...
VALUES ('$num_expediente', '$tmp[$i]' ," . $sqlFechaHoy . ",'$depe_codi' ,'$usua_codi' ,'$usua_doc','$estado_expediente')";
                    $rs = $this->db->conn->query($query);
                    $i++;
                }
            } else {
                //Si no trae coma solo viene uno
                $query = "insert into SGD_EXP_EXPEDIENTE(SGD_EXP_NUMERO, RADI_NUME_RADI,SGD_EXP_FECH,DEPE_CODI,USUA_CODI,USUA_DOC,SGD_EXP_ESTADO )
VALUES ('$num_expediente', '$radicado' ," . $sqlFechaHoy . ",'$depe_codi' ,'$usua_codi' ,'$usua_doc','$estado_expediente')";
                $rs = $this->db->conn->query($query);
                echo "Solo venia un radicado";
            }
            ...
            if (!$rs) {
                ...
                $q_excluido .= " AND RADI_NUME_RADI in ('$tmp[$i]' )";
                // print $q_excluido;
                ...
                if ($rs_excluido->fields["sgd_exp_estado"] == 2) {
                    ...
                    $q_update .= " AND RADI_NUME_RADI == ('$tmp[$i]')";
...
//$this->db->conn->debug=true;
    ...
    function consultaTipoExpediente($numExpediente) {
        $query = "select se.SGD_EXP_NUMERO as SGD_EXP_NUMERO
                            , sb.SGD_SRD_CODIGO as SGD_SRD_CODIGO
                            , sr.SGD_SRD_DESCRIP as SGD_SRD_DESCRIP
                            , sb.SGD_SBRD_CODIGO as SGD_SBRD_CODIGO
                            , sb.SGD_SBRD_DESCRIP as SGD_SBRD_DESCRIP
                            , se.SGD_FEXP_CODIGO as SGD_FEXP_CODIGO
                            , se.SGD_SEXP_FECH as SGD_SEXP_FECH
                            , se.USUA_DOC_RESPONSABLE as USUA_DOC_RESPONSABLE
                            , se.SGD_PEXP_CODIGO as SGD_PEXP_CODIGO
...
//		$cuentaEtapas=-1;
        ...
        if ($numExpediente) {
            ...
            $query = "SELECT SGD_PEXP_DESCRIP as SGD_PEXP_DESCRIP, SGD_PEXP_CODIGO, SGD_PEXP_TERMINOS as SGD_PEXP_TERMINOS, SGD_PEXP_AUTOMATICO as SGD_PEXP_AUTOMATICO FROM SGD_PEXP_PROCEXPEDIENTES
		      WHERE SGD_SRD_CODIGO= " . $this->codiSRD . " AND SGD_SBRD_CODIGO=" . $this->codiSBRD;
            ...
            IF ($this->codigoFldExp != 0) {
                $query = "SELECT SGD_FEXP_DESCRIP as SGD_FEXP_DESCRIP
                            , SGD_FEXP_CODIGO as SGD_FEXP_CODIGO
                            , SGD_FEXP_TERMINOS as SGD_FEXP_TERMINOS
...
//            	die("Cuenta 2: " . $rs->FieldCount()  );
    ...
    function secExpediente($dependencia, $codiSRD, $codiSBRD, $anoExp) {
        $query = "select se.SGD_EXP_NUMERO as SGD_EXP_NUMERO
                    , se.SGD_FEXP_CODIGO as SGD_FEXP_CODIGO
                    , se.SGD_SEXP_SECUENCIA as SGD_SEXP_SECUENCIA
                from SGD_SEXP_SECEXPEDIENTES se
                WHERE
                    SGD_SRD_CODIGO=$codiSRD
        ...
        $rs = $this->db->conn->Execute($query);
//        error_log('-------------------------> archivo tx/expediente linea 623');
        $numExpediente = $rs->fields["SGD_EXP_NUMERO"];
    ...
    function expedientesRadicado($radicado) {
        ...
        $query .= "   WHERE RADI_NUME_RADI in ($radicado)";
        $query .= " AND SGD_EXP_ESTADO <> 2";
        $rs = $this->db->conn->query($query);
        //error_log('********************** ExpedienteMultiple.php '.$query);
        if ($rs->EOF) {
            $arrExpedientes[0] = 0;
        } else {
            $e = 0;
    ...
    function expedienteAnexoAsociado($radicado) {
        ...
        $query .= '   AND RADI_NUME_SALIDA <> i\'' . $radicado . "'";
        $query .= ' )';
        // print $query;
    ...
    function trdExpediente($numExpediente) {
        $query = "select SGD_SRD_CODIGO, SGD_SBRD_CODIGO from SGD_SEXP_SECEXPEDIENTES where SGD_EXP_NUMERO='$numExpediente'";
        $rs = $this->db->conn->query($query);
        $arrExpArchivo['serie'] = $rs->fields['SGD_SRD_CODIGO'];
        $arrExpArchivo['subserie'] = $rs->fields['SGD_SBRD_CODIGO'];
