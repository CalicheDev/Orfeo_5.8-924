...
if (!defined('_ADODB_LAYER')) {
    ...
    var $_oldRaiseFn = false;
        ...
        function Connect($argHostname = "", $argUsername = "", $argPassword = "", $argDatabaseName = "", $forceNew = false) {
            ...
            if ($argPassword != "")
                $this->password = $argPassword; // not stored for security reasons
            if ($argDatabaseName != "")
        ...
        /**
         * PEAR DB Compat - do not use internally. 
         * Función modificada con el fin se controllar el GeneraID para las secuencias unicamente en este archivo
         */
        function nextId($seq_name, $driver) {
            //return $this->GenID($seq_name);
            error_log('---> nexid ----> Linea 562 ' . $seq_name . ' ----> ' . $driver);
            return $this->GeneraID($seq_name, $driver);
        }

        /**
         * Por Skinatech - jmgamez@skinatech - 13/02/2018
         * Nueva función para realizar las secuencias segun el driver que se configure en el archivo config.php 
         * */
        function GeneraID($seq_name, $driver) {
            if ($this->conn->hasGenID) {
//                error_log('----------->  adodb ---> Linea 570 ' . $seq_name);
                return $this->conn->GenID($seq_name, 1);
            } else {
                switch ($driver) {
                    case 'mssql':
                        $q = "select next value for $seq_name as SEC";
                        $rs = $this->query($q);
                        if (!$rs->EOF) {
                            if (isset($rs->fields['SEC'])) {
                                $secu = $rs->fields['SEC'];
                            } else {
                                $secu = $rs->fields[0];
                            }
                            error_log('******** ' . $rs->fields['SEC'] . ' ******* ' . $rs->fields[0]);
                            $retorno = $secu;
                            error_log("Retorna en la funcion de secuencia($retorno) -------> adodb ---> linea 580 query " . $q);
                        }
                        break;
                    case 'ocipo':
                        $q = "select $seq_name.nextval as SEC from dual";
                        $rs = $this->query($q);
                        if (!$rs->EOF) {
                            $retorno = $rs->fields['SEC'];
                            error_log("Retorna en la funcion de secuencia($retorno) -------> adodb ---> linea 589 query " . $q);
                        }
                        break;
                    case 'mysql':
                        $q = "select seq_val as SEC FROM _sequence where seq_name='$seq_name'";
                        $rs = $this->query($q);
                        if (!$rs->EOF) {
                            $netVal = $rs->fields['SEC'] + 1;
                            $retorno = $rs->fields['SEC'];
                            $qs = "update _sequence set seq_val=" . $netVal . " where seq_name='$seq_name'";
                            $rsqs = $this->query($qs);
                            $rs = $this->query($q);
                            error_log("Retorna en la funcion de secuencia($retorno) -------> adodb ---> linea 608 query " . $q);
                        }
                        break;
                    case 'postgres':
                        $q = "select nextval('$seq_name') as SEC";
                        $rs = $this->query($q);
                        if (!$rs->EOF) {
                            $retorno = isset($rs->fields['SEC']) ? $rs->fields['SEC'] : $rs->fields['sec'];
                            error_log("Retorna en la funcion de secuencia($retorno) -------> adodb ---> linea 598 query " . $q);
                        }
                        break;
                }
                return $retorno;
        ...
        function &Execute($sql, $inputarr = false) {
            //Modificado por Openous
            $this->create_trail($sql);
            if ($this->fnExecute) {
                $fn = $this->fnExecute;
        ...
        function create_trail($sql) {
            ...
            if ($arrayQuery[0] != "SELECT") {
                ...
                if (strcmp($table, "SGD_AUDITORIA") != 0 && isset($type) && $type != '') {
                    $sql = str_replace("'", "^", $sql);
                    $query = "INSERT INTO SGD_AUDITORIA (FECHA, USUA_DOC, IP, TIPO, TABLA, ISQL) VALUES ('" . time() . "', '" . $_SESSION['usua_doc'] . "', '$realip', '$type', '$table','$sql')";
                    $this->Execute($query);
                }
            }
        }
        ...
        function _gencachename($sql, $createdir) {
            ...
            if ($createdir && $notSafeMode && !file_exists($dir)) {
                ...
                if (!mkdir($dir, 0771))
                    if ($this->debug)
                        ADOConnection::outp("Unable to mkdir $dir for $sql");
                umask($oldu);
            }
    ...
    class ADORecordSet extends ADODB_BASE_RS {
        ...
        function GetMenu2($name, $defstr = '', $blank1stItem = true, $multiple = false, $size = 0, $selectAttr = '') {
            ...
            global $ADODB_INCLUDED_LIB;
            //PASO INICIAL CORRECCION MENU cmartinez@skinatech.com
            if (empty($ADODB_INCLUDED_LIB))
                include_once(ADODB_DIR . '/adodb-lib.inc.php');
    ...
    function ADOLoadCode($dbType) {
        ...
        switch ($db) {
            case 'ifx':
            case 'maxsql': $db = 'mysqlt';
                break;
            case 'postgres':
            case 'pgsql': $db = 'postgres7';
                break;
        }
